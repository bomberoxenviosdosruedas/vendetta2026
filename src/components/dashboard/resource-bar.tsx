...
function ResourceValueCell({ res }: { res: ResourceEntry }) {
  ...
  return (
    <td
      className={cn(
        "vb-cell border border-[#000] text-[10px] !py-0.5 px-1.5 whitespace-nowrap text-center font-['JetBrains_Mono'] font-bold tabular-nums transition-colors",
...
function MobileResourceTile({ res, index, booted, isOpen, onToggle }: {
  ...
  return (
    ...
    >
      ...
      <div className="vb-crimson-cell flex items-center justify-between px-1.5 py-0.5 text-[9px] font-['Chivo'] font-bold uppercase tracking-wider">
      ...
      <div className="flex items-center justify-between px-1.5 py-0.5 bg-[#17130d]">
        <span
          className={cn(
            "text-[11px] font-['JetBrains_Mono'] font-bold tabular-nums leading-none",
        ...
        <span className="text-[8px] text-[#8a7d63] font-['JetBrains_Mono']">
      ...
      <div className={cn(
        ...
        <div className="overflow-hidden">
          <div className="px-1.5 py-1 border-t border-[#2c2418] bg-[#120e09] text-[8px] font-['JetBrains_Mono'] leading-relaxed">
...
export function ResourceBar({ user, variant, onOpenMenu }: ResourceBarProps) {
  ...
  return (
    <header className="w-full h-12 min-h-12 shrink-0 z-50 flex items-center justify-between gap-3 px-3 xl:px-4 border-b-2 border-[#5a4b33] shadow-[0_4px_12px_rgba(0,0,0,0.9)] bg-[linear-gradient(180deg,#443c2c_0%,#2a2418_50%,#16120b_100%)]">
      ...
      {/* Panel de instrumentos: ticker + expediente expandible */}
      <div className="relative flex-1 max-w-4xl mx-auto px-1 min-w-0 group">
        <div
          role="button"
          tabIndex={0}
          aria-expanded={open}
            ...
            <tbody>
              ...
              <tr>
                {resources.map((res) => (
                  <th
                    key={res.key}
                    scope="col"
                    className="vb-crimson-cell text-center text-[10px] font-['Chivo'] font-bold uppercase tracking-wider !py-0.5 px-1 whitespace-nowrap w-[20%] border border-t-0"
                  >
                    {res.name}
                  </th>
                ))}
                <th
                  scope="col"
                  className="vb-crimson-cell text-center text-[9px] font-['JetBrains_Mono'] font-bold !py-0.5 px-1 whitespace-nowrap hidden sm:table-cell w-[20%] border border-t-0"
                >
                  HORA (SERVIDOR)
                </th>
              </tr>
              ...
              <tr>
                {resources.map((res, i) => {
                  ...
                  return (
                    <td key={res.key} className="vb-track !p-0 h-[6px] border border-[#000]">
                      <div
                        className={cn("vb-fill h-full transition-[width] duration-700 ease-out", isFull && "vb-full-pulse")}
                        style={{
                          width: booted ? `${percentage}%` : '0%',
                ...
                <td className="vb-track !p-0 h-[6px] border border-[#000] hidden sm:table-cell" />
              </tr>
              ...
              <tr>
                ...
                <td className="vb-cell border border-[#000] text-[9px] !py-0.5 px-2 hidden sm:table-cell text-center font-['JetBrains_Mono']">
                  <span className="inline-flex items-center gap-1">
                    <LiveClock />
                    <span className={cn("text-[#ffe569] transition-transform", open ? "rotate-180" : "")}>
                      <MaterialIcon name="expand_more" size={11} />
                    </span>
                  </span>
                </td>
              </tr>
                  ...
                  <tr key={row.label}>
                    {resources.map((res) => (
                      <td
                        key={res.key}
                        className={cn(
                          "vb-cell border border-[#000] text-[9px] !py-0.5 px-1.5 whitespace-nowrap text-center font-['JetBrains_Mono']",
                    ...
                    {rowIdx === 0 && (
                      <td className="vb-cell border border-[#000] text-[9px] px-2 hidden sm:table-cell align-middle text-left" rowSpan={4}>
                        <div className="space-y-0.5">
                          <div className="text-[8px] text-[#8a7d63] font-['JetBrains_Mono'] uppercase tracking-wider">Coordenadas</div>
                          <div className="text-[#dfcca0] font-['JetBrains_Mono'] font-bold">{coords}</div>
                          <div className="pt-0.5 text-[8px] text-[#8a7d63] font-['JetBrains_Mono'] uppercase tracking-wider">Hora servidor</div>
                          <LiveClock />
                          <div className="pt-1 text-[8px] text-[#ffe569]/80 font-['JetBrains_Mono'] uppercase tracking-wider">
